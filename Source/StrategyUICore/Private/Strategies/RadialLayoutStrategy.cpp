...
//----------------------------------------------------------------------------------------------
void URadialLayoutStrategy::InitializeStrategy(const TScriptInterface<ILayoutStrategyHost> Host)
{
	if (!ensureMsgf(Host, TEXT("Host is null in %hs!"), __FUNCTION__))
	{
		return;
	}
	SetNumItems(ILayoutStrategyHost::Execute_GetNumItems(Host.GetObject()));
